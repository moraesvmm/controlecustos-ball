...
if exist "%PROJECT_ROOT%\.python_local\tools\python.exe" (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -Command "$p = Start-Process -FilePath '%PROJECT_ROOT%\.python_local\tools\python.exe' -ArgumentList '-m','http.server','%PORT%','--bind','0.0.0.0' -WorkingDirectory '%PROJECT_ROOT%' -WindowStyle Hidden -PassThru; Set-Content -LiteralPath '%PID_FILE%' -Value $p.Id"
    if errorlevel 1 (
